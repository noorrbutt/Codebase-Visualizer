        ...
        <form onSubmit={handleSubmit} style={{ marginBottom: "1.5rem" }}>
          <div style={{ display: "flex", gap: 0, border: "1.5px solid #E5E7EB", borderRadius: 10, overflow: "hidden", background: "#fff", transition: "border-color 0.15s", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ padding: "14px 24px", background: "#111", color: "#fff", border: "none", cursor: loading ? "not-allowed" : "pointer", fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 500, whiteSpace: "nowrap", opacity: loading ? 0.6 : 1 }}
            >
  ...
  const startPolling = (id) => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (json.summary) setData((prev) => ({ ...prev, summary: json.summary, status: json.status }));
