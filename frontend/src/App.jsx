      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5, flex: 1 }}>
        {entries.map(([lang, count]) => (
          <div key={lang} style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: "#9CA3AF" }}>{((count / total) * 100).toFixed(0)}%</span>
          </div>
        ))}
      </div>
    </div>
...
function ConnectionsChart({ nodes, edges }) {
  ...
  useEffect(() => {
    ...
    vals.forEach((v) => {
      const b = Math.min(Math.floor((v / (maxConn + 1)) * 8), 7);
      bins[b]++;
    });
...
// ── Node detail panel ──────────────────────────────────────────────────
function NodeDetail({ node, edges, nodes, onClose }) {
  if (!node) return null;
...
export default function DashboardPage({ data, onReset }) {
  ...
  // compute visible nodes/edges for graph tab
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#FAFAF9" }}>
      ...
      <header style={{ height: 52, borderBottom: "1px solid #E5E7EB", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 1.25rem", background: "#fff", flexShrink: 0, zIndex: 20 }}>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: "#6B7280", textDecoration: "none", display: "flex", alignItems: "center", gap: 4 }}
        >
          GitHub ↗
        </a>
      </header>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 10, padding: "14px 16px" }}>
              ...
              {(() => {
                ...
                const total = entries.reduce((s, [, c]) => s + c, 0);
                if (!entries.length) return <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: "#9CA3AF", margin: 0 }}>AI role analysis not yet available — open the graph and click a file to trigger it.</p>;
                return (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {entries.slice(0, 8).map(([role, count]) => (
                      <div key={role} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <div style={{ width: 80, height: 4, background: "#F3F4F6", borderRadius: 2 }}>
                          <div style={{ width: `${(count / entries[0][1]) * 100}%`, height: "100%", background: "#6366F1", borderRadius: 2 }} />
                        </div>
      ...
      {activeTab === "graph" && (
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          ...
          <NodeDetail node={selectedNode} edges={data.edges} nodes={data.nodes} onClose={() => setSelectedNode(null)} />
        </div>
      )}
      ...
      {activeTab === "files" && (
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
            <FileListPanel nodes={data.nodes} edges={data.edges} onSelectNode={(n) => { setSelectedNode(n); }} />
          </div>
          <NodeDetail node={selectedNode} edges={data.edges} nodes={data.nodes} onClose={() => setSelectedNode(null)} />
        </div>
      )}
